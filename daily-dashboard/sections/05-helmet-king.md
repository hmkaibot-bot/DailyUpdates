...
設定見 config `monthly_targets`（零售 140萬營 / 車房 40萬營 / 賣車 25萬純利 / 租車 2萬純利）。
旅行團：**報數但不設 target**（來源待確認，見 config sources「旅行團」）。
通用公式（以最後完整日 LCD 計）：
```
LCD          = current_date - 1
...
## 集團總 GP（所有部組相加）—— ⚠️ 必算，絕不可留空

設定見 config `group_profit`。**月目標 GP = HK$1,000,000**。

**計算步驟（必做）**：先把 5 個分項各算出一個數字，再相加成一個總額，最後對 100萬目標算達成%。**即使某分項為 0 或缺資料，也要用可得分項相加並照常輸出一個總數字——這一行永不留空。**
```
gp_retail   = 零售 MTD 營業額 × 0.34
gp_garage   = 車房 MTD 零件貢獻毛利 = Q3b 嘅 g_gp   (真實；⚠️ 未扣師傅人工，見下)
...
gp_property = 租屋收入 固定 $52,000            (config group_profit.property_rent)
─────────────────────────────────────────────
...
- **車房已改用實數**（2026-09-01 起）：`gp_garage` = Q3b 嘅 `g_gp`，唔再用 48% 假設。
  ⚠️ 但佢係**零件貢獻毛利、未扣師傅人工**（工時佔收入約 57.5% 而成本計 0），
  同其餘分項層次唔同 —— 輸出時要標明「未計人工」，唔好講成「真實毛利率 71.4%」。
  2026-08-31 或之前嘅歷史期間，車房仍用 BC 營業額 × 48%。
...
> ⚠️ $1M 目標當初係按「車房 BC $365k × 48% ≈ $175k」校出；車房換咗口徑之後，$1M 本身亦要重校（config `garage.caveat`）。
...
旅行團          $XXX         —        報數      —         （無 target）
租屋收入        $52,000      —        報數      —         （固定，無 target）
━━ 集團總 GP(估)  $800,617     $1.0M    80.1%    $199,383  ← 必填實際算出的數字，不可空白
👉 跨線洞察（哪條最落後、最該追）
```
> 上表「集團總 GP」一行是**必填**：把五個分項數字相加後填入，連同 達成% 與 距標。若漏算＝報告有缺陷。
