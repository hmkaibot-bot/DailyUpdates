...
設定見 config `group_profit`。**月目標 GP = HK$1,000,000**（追數同通用公式）。
```
集團總 GP(MTD) ≈ 零售營業額×34%(假設)
              + 車房營業額×48%(假設)
              + 賣車已實現純利(真實)
              + 租車淨利(真實)
              + 保險利潤(真實，目前資料停更計 $0)
```
- **零售/車房 DB 無成本(COGS)** → 用毛利率假設(34%/48%，config 可改)，必附 caveat。
- 保險：主來源 = **CostGo 續保** `costgo_renewal_cases`（活躍，6月 43 宗、gross_premium $62,725）。**純利暫算不到**（sell/cost 未填）→ 需老闆給佣金基準（gross_premium×X% 或 renewal_price−cost）。現先**報量**，純利暫計 $0。
- 旅行團：併入報數（無 target，來源待確認）。
> 驗證(6/27)：零售 ~$411,761 + 車房 ~$146,665 + 賣車 $196,824 + 租車 $3,640 + 保險純利 $0(待佣金基準) ≈ **$758,890** / 目標 100萬（76%，應達 90% → 落後）。保險佣金基準確定後此數會上升。
...
旅行團          $XXX         —        報數      —         （無 target）
— 集團總 GP(估)   $XXX,XXX     $1.0M    {%}      $XXX      （零售/車房毛利估算）
👉 跨線洞察（哪條最落後、最該追）
```
