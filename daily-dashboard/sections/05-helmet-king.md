...
─────────────────────────────────────────────
集團總 GP(MTD) = gp_retail + gp_garage + gp_sales + gp_rental + gp_insurance   ← 必輸出此數字
...
> 範例驗算(6/27)：411,761 + 146,665 + 196,824 + 3,640 + 9,409 = **$768,299** / 100萬 = 76.8%（應達 90% → 落後）。實際每日重算，數字會變，但**格式固定要有總數**。
...
旅行團          $XXX         —        報數      —         （無 target）
━━ 集團總 GP(估)  $768,299     $1.0M    76.8%    $231,701  ← 必填實際算出的數字，不可空白
👉 跨線洞察（哪條最落後、最該追）
```
